...
if ($bbb == "ok" ) {
 ...
 header('Location:t_selete_query.php?sort=name_sort');
  ...
  $Q = "SELECT   `note`,`note1`,`note2`,`note3`,`photo1`,`photo2`,`photo3`,`date`,`tel` FROM `photos` WHERE `datetime` ='{$diagnose}' ORDER BY `photos`.`date` DESC" ;
...
$up = '<div style="overflow-y:auto;height:auto;max-height: 830px;">';
  ...
  $up.='<img   src="image/plus.png" style="width: 30px;height: auto;">';                        
  $up.='</div>';
  $up .= '</div>';
  ...
  $ta .= '</div>'; 
   ...
   // Modal content 
  $close_model='<div class="modal-content" style="font-size: 18px; color: black;text-align: justify;">';
  $close_model.='<div class="modal-header">';
  $close_model.='<a href="'.$url.'">'.'<span class="close">'.'&times;'.'</span>'.'</a>';
  $close_model.='<h2>'.$all['date'].'</h2>';
  $close_model.='</div>';
  $close_model.='<div class="modal-body">';
  if (!empty($note1)) {
  $close_model.= '<div class="diagnose-list" style="width: 100%; height: 350px;">';
        $close_model.= '<div class="discription" style="float: left; width: 55%; ">';
        $close_model.= '<p>'.$note1.'</p>';
        $close_model.= '</div>';
  if (!empty($pic1)) {
        $close_model.= '<div class="diagnose-photo" style="float: right; width: 40%;">';
          $close_model.=  $pic1;
        $close_model.= '</div>';
  }
  }
      ...
      $close_model.= '</div><br><br>';
      $close_model.= '<div class="diagnose-main" style="width: 100%; height: auto;">';
      $close_model .= '<p>'.$note.'</p>';
      $close_model.= '</div><br><br>';
      $close_model.= '<div class="report">';
        $close_model.= '<h3>Laboratary Reports</h3>';
        $close_model.= '<ol>';
          $close_model.= '<li>Blood Pressure <a href="#">></a></li>';
          $close_model.= '<li>Blood Pressure <a href="#">></a></li>';
          $close_model.= '<li>Blood Pressure <a href="#">></a></li>';
          $close_model.= '<li>Blood Pressure <a href="#">></a></li>';
        $close_model.= '</ol>';
      $close_model.= '</div>';
  $close_model.='</div>';
  $close_model.='<div class="modal-footer">';
  $close_model.='</div>';
  $close_model.='</div>';
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
    ...
    <link rel="stylesheet" type="text/css" href="CSS/view_diagnose.css">
